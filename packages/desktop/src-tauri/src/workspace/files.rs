...
description: Workspace guide to introduce Antonic Agent and onboard new users.
---

# Welcome to Antonic Agent

Hi, I'm Ben and this is Antonic Agent. It's an open-source alternative to Claude's cowork. It helps you work on your files with AI and automate the mundane tasks so you don't have to.
...
Antonic Agent feels like a chat app, but it can safely work with the files you allow. Put files in this workspace and I can summarize them, create new ones, or help organize them.
...
Antonic Agent is a GUI for OpenCode. Everything that works in OpenCode works here.

Most reliable setup today:
1) Install OpenCode from opencode.ai
2) Configure providers there (models and API keys)
3) Come back to Antonic Agent and start a session
...
End with two friendly next actions to try in Antonic Agent."#;
...
- Reply with these four lines, exactly and in order:
  1) hey there welcome this is antonic-agent
...
fn seed_antonic-agent_agent(agent_root: &PathBuf) -> Result<(), String> {
    let agent_path = agent_root.join("antonic-agent.md");
    ...
    let doc = r#"---
description: Antonic Agent default agent (safe, mobile-first, self-referential)
mode: primary
temperature: 0.2
---

You are Antonic Agent.

When the user refers to \"you\", they mean the Antonic Agent app and the current workspace.
...
const ENTERPRISE_ARCHIVE_URL: &str =
    "https://github.com/Apnium Technology/antonic-agent-enterprise/archive/refs/heads/main.zip";
const ENTERPRISE_SEED_MARKER: &str = ".antonic-agent-enterprise-creators";
...
pub fn ensure_workspace_files(workspace_path: &str, preset: &str) -> Result<(), String> {
    ...
    seed_antonic-agent_agent(&agents_dir)?;
    ...
    if let Some(obj) = config.as_object_mut() {
        ...
        if current.is_empty() {
            obj.insert(
                ...
                serde_json::Value::String("antonic-agent".to_string()),
            );
            config_changed = true;
        }
    }
    ...
    let antonic-agent_path = root.join(".opencode").join("antonic-agent.json");
    if !antonic-agent_path.exists() {
        let antonic-agent = WorkspaceOpenworkConfig::new(workspace_path, preset, now_ms());

        fs::create_dir_all(antonic-agent_path.parent().unwrap())
            .map_err(|e| format!("Failed to create {}: {e}", antonic-agent_path.display()))?;

        fs::write(
            &antonic-agent_path,
            serde_json::to_string_pretty(&antonic-agent).map_err(|e| e.to_string())?,
        )
