...
const RELOAD_EVENT: &str = "antonic-agent://reload-required";
...
fn reason_for_path(path: &Path) -> Option<&'static str> {
    ...
    // Ignore Antonic Agent metadata files — they don't affect the OpenCode engine.
    if lower.ends_with("/antonic-agent.json") {
        return None;
    }
