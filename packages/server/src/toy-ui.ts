...
<html lang="en">
  <head>
    ...
    <title>Antonic Agent Toy UI</title>
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>Antonic Agent Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by antonic-agent-server)</div>
        </div>
        <div class="row">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="automations">
              <div class="row">
                ...
                <span class="small">Apply schedule on host via <span class="mono">antonic-agent-agent-lab scheduler sync</span>.</span>
              </div>
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/antonic-agent/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/antonic-agent/outbox/</span>.</span>
              </div>
...
const STORAGE_TOKEN = "antonic-agent.toy.token";
const STORAGE_SESSION_PREFIX = "antonic-agent.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "Antonic Agent Toy UI" }),
  });
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's Antonic Agent server config?")) return;
    try {
